...
formula = y ~ skew_normal(1/mean ~ a + b + c^2 + c,
                          log(sd) ~ b + c)
...
stanny = function(formula, data, p_formula, mean_priors = NULL, sd_priors = NULL, p_priors = NULL) {
...
stanny(formula, data = as.data.frame(mtscars), p_formula, mean_priors, sd_priors, p_priors) ->
  mod
